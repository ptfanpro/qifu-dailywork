...
export async function observeCodeModel({source,read,reader}){
 const bytes=Buffer.from(source),base=structuredClone(read),photoSha256=hash(bytes);
 if(photoSha256!==base.inputSha256||reader.modelSha256!==CHINESE_BODY_MODEL_SHA256)
 ...
 const report={schemaVersion:1,recipe:'chinese-model-original-code-crops-v1',completed:true,
  modelSha256:CHINESE_BODY_MODEL_SHA256,photoSha256,baseReadSha256:digest(base),rows,
