...
export async function reviewCurrentPdfBodies({appRoot,pdfFiles,pdfPages,pdfIndexBinding,claims,onProgress=null,
  createReader=createChineseBodyReader,loadPages=null}) {
  ...
  try {
      ...
      for(const file of pdfFiles) {
        ...
        try {
          for(let pageNumber=1;pageNumber<=document.numPages;pageNumber++) {
            ...
            readings.push({pdfSha256,pageNumber,views:await reader.read(canvas.toBuffer('image/png'),{includeVertical:true})});
            page.cleanup();
    ...
    for(let i=0;i<claims.length;i++) {
      ...
      try {
        ...
        assessment=assessBodyClaim(await reader.read(fs.readFileSync(claim.file),{includeVertical:true}),pages,target);
  ...
  return {status:'veto-only-not-order-binding',attempted:claims.length,
    blocked:claims.filter(bodyReviewBlockReason).length,seconds:(Date.now()-started)/1000,results};
}
