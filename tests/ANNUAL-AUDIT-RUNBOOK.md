...
参照图角色专项见 `experiments/REFERENCE-ROLE-EXPERIMENT.md`。固定图像模型、人工逐图审核参照、按整日留出并拒绝相同内容，仍只输出诊断。首轮 55 张参照 / 72 张留出中六张水景全未确定，因此没有接入产品。不得按历史文件名生成校准标签，特征向量和真实图片同样禁止入 Git。四项协议回归已纳入普通入口；源码/参照标签与测试答案分别保存，不能通过查看答案后改参照库而继续称为同一留出验收。

等日期参照对照须显式传 `--balanced-dates`，不替换原全局邻居轮次。新增 71 张完整日期留出只证明本批 69 项同类、2 项未决；扩大到其余历史日期仍是角色诊断。新四项协议回归与旧四项同时运行；不得在见到失败后选择成功视图、把全图特征当完整前景证明，或把类别候选写入上传/赋号计划。原件、向量、真实标签、相近图身份及所有报告只存本机私有 TEMP；保存向量复算不计为新的图像读取或独立样本。
