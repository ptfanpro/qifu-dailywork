// Read-only local acceptance tool. No images or customer content in its output.
import fs from 'node:fs';
import path from 'node:path';
import crypto from 'node:crypto';
import {diagnosePhotoStructure,isLikelyScene} from '../src/photo-prepare.mjs';
const report={checked:0,changedScenes:0,changedNumberedPapers:0,changedUnknown:0,changes:[]};
for(const dir of process.argv.slice(2)) {
  for(const file of fs.readdirSync(dir).filter(n=>/\.jpe?g$/i.test(n))) {
    const full=path.join(dir,file);
    const item=await diagnosePhotoStructure(full);
    const before=isLikelyScene({...item,sceneMetrics:{...item.sceneMetrics,flameStructure:undefined}});
    report.checked++;
