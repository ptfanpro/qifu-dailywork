...
const sourceFiles = ['tests/experiments/pdf-visual-body-probe.mjs', 'tests/experiments/pdf-visual-body-evidence.mjs',
  ...
  'tests/experiments/text-regions.mjs', 'src/local-ocr.mjs'];
...
try {
  for (const pdf of day.pdfs) {
    ...
    try {
      for (let pageNumber = 1; pageNumber <= document.numPages; pageNumber++) {
        ...
        pages.push({pdfSha256: pdf.sha256, pageNumber, text});
  ...
  for (const photo of selected) {
    ...
    fs.writeFileSync(path.join(output, `${photo.sha256}-report.json`), JSON.stringify({
      seconds: (Date.now() - tick) / 1000, textOnly: reports.textOnly.at(-1), visual: reports.visual.at(-1)}, null, 2));
  ...
  console.log(JSON.stringify({privateOutput: output, date, pages: pages.length, photos: selected.length,
    seconds: summary.seconds, textOnly: summary.textOnly.counts, visual: summary.visual.counts, sourceUnchanged: true}));
} catch (error) {
