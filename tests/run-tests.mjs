...
assert.match(uiSource,/AutomationCredentialStore\.ps1/);
assert.match(uiSource,/UseSystemPasswordChar = \$true/);
assert.match(uiSource,/secure-login\.dat/);
assert.match(uiSource,/浏览器辅助登录：已配置/);
assert.match(uiSource,/secure-automation\.dat/);
assert.match(uiSource,/机器接口：已配置/);
assert.match(uiSource,/Start-Runner 'automation-auth-check' \$false 'automation-auth'/);
assert.match(uiSource,/接口认证不创建后台人员会话，也不会绕过验证码/);
